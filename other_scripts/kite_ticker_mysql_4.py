...
import logging
from Fetch_Ticker_Data_Scripts.kite_ticker_mysql_sandbox import *
import pandas as pd
...
kws = KiteTicker(key_secret[0], data["access_token"], debug=True)
timeout = time.time() + 60 * 4.5  # 5 minutes from now
...
while True:
    ...
    if timer == 4.5 or time.time() > timeout:
        break
    timer = timer - 1
    time.sleep(5)
